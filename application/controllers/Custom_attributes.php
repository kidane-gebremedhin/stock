...
class custom_attributes extends Admin_Controller 
{
	public function __construct()
	{
		...
		$this->data['page_title'] = 'custom_attributes';
	...
	*/
	public function index()
	{
		...
		$this->render_template('custom_attributes/index', $this->data);	
	}

	/* 
	* fetch the attribute data through attribute id 
	*/
	public function fetchCustomAttributeDataById($id) 
	{
		if($id) {
			$data = $this->model_custom_attributes->getAttributeData($id);
	...
	*/
	public function fetchCustomAttributeData()
	{
		$result = array('data' => array());

		$data = $this->model_custom_attributes->getAttributeData();

		foreach ($data as $key => $value) {

//			$count_attribute_value = $this->model_custom_attributes->countAttributeValue($value['id']);
	...
	public function create()
	{
        ...
        if ($this->form_validation->run() == TRUE) {
        	$data = array(
        		...
        		'is_number' => $this->input->post('is_number'),
        		'active' => $this->input->post('active'),	
        	);

        	$create = $this->model_custom_attributes->create($data);
        	if($create == true) {
	...
	public function update($id)
	{
		...
		if($id) {
	        ...
	        if ($this->form_validation->run() == TRUE) {
	        	...
	        	$update = $this->model_custom_attributes->update($data, $id);
	        	if($update == true) {
	        		$response['success'] = true;
	...
	*/
	public function remove()
	{
		...
		$response = array();
		if($attribute_id) {
			$delete = $this->model_custom_attributes->remove($attribute_id);
			if($delete == true) {
				$response['success'] = true;
	...
	*/
	public function addvalue($attribute_id = null)
	{
		...
		$this->data['attribute_data'] = $this->model_custom_attributes->getAttributeData($attribute_id);

		$this->render_template('custom_attributes/addvalue', $this->data);	
	}
	...
	*/
	public function fetchAttributeValueData($attribute_parent_id)
	{
		...
		$data = $this->model_custom_attributes->getAttributeValueData($attribute_parent_id);
	...
	*/
	...
	{
		if($id) {
			$data = $this->model_custom_attributes->getAttributeValueById($id);
			echo json_encode($data);
		}
	}
	...
	*/ 
	public function createValue()
	{
        ...
        if ($this->form_validation->run() == TRUE) {
        	...
        	$create = $this->model_custom_attributes->createValue($data);
        	if($create == true) {
        		$response['success'] = true;
        		$response['messages'] = 'Succesfully created';
        	}
	...
	public function updateValue($id)
	{
		...
		if($id) {
	        ...
	        if ($this->form_validation->run() == TRUE) {
	        	...
	        	$update = $this->model_custom_attributes->updateValue($data, $id);
	        	if($update == true) {
	        		$response['success'] = true;
	        		$response['messages'] = 'Succesfully updated';
	        	}
	...
	*/
	public function removeValue()
	{
		...
		if($attribute_value_id) {
			$delete = $this->model_custom_attributes->removeValue($attribute_value_id);
			if($delete == true) {
				$response['success'] = true;
				$response['messages'] = "Successfully removed";	
			}
